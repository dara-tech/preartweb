...
function PatientList({
  ...
  const { isMultiSite, selectedSite: contextSelectedSite } = useSite()
  const effectiveSelectedSite = selectedSite || contextSelectedSite
  ...
  const loadLookupData = async () => {
    try {
      ...
      const siteCode = effectiveSelectedSite?.code || null
      ...
      setTargetGroups(Array.isArray(targetGroupsRes.data) ? targetGroupsRes.data : [])
    } catch (error) {
      ...
      <div className="bg-card border-b border-border">
        <div className="px-4 sm:px-6 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-foreground">Adult Patients</h1>
        ...
        {loading ? (
          ...
          <Card className="border border-border shadow-none bg-card">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-6">
                {searchTerm ? 'Try adjusting your search or filters' : 'No adult patients registered yet'}
              </p>
          ...
          <div className="space-y-3">
            {patients.map((patient, index) => (
              <Card 
                key={`${patient.clinicId}-${patient.site_code || patient.siteName || 'unknown'}-${index}`} 
                ...
                <CardContent className="p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0 space-y-3">
                      ...
                      {(patient.targetGroup || patient.nationality) && (
                        <div className="flex items-center gap-3 text-xs flex-wrap">
                          ...
                          {patient.nationality !== null && patient.nationality !== undefined && patient.nationality !== '' && patient.nationality !== -1 && patient.nationality !== 0 && getNationalityLabel(patient.nationality) && (
                            <span className="px-2 py-1 bg-gray-100 text-foreground rounded">
