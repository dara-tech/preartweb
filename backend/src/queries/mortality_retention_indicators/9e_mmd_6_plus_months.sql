-- ===================================================================
-- Indicator 9e: Percentage of ART patients with MMD ≥6 months
-- ===================================================================
...
mmd_stats AS (
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN ndays >= 161 THEN ClinicID END) AS Six_Plus_Months,
        ...
        COUNT(DISTINCT CASE WHEN type = 'Child' AND Sex = 'Male' AND ndays >= 161 THEN ClinicID END) AS Male_0_14_6M_Plus,
        COUNT(DISTINCT CASE WHEN type = 'Child' AND Sex = 'Female' AND ndays >= 161 THEN ClinicID END) AS Female_0_14_6M_Plus,
        COUNT(DISTINCT CASE WHEN type = 'Adult' AND Sex = 'Male' AND ndays >= 161 THEN ClinicID END) AS Male_over_14_6M_Plus,
        COUNT(DISTINCT CASE WHEN type = 'Adult' AND Sex = 'Female' AND ndays >= 161 THEN ClinicID END) AS Female_over_14_6M_Plus
    FROM active_patients
)

SELECT
    '9e. Percentage of ART patients with MMD ≥6 months' AS Indicator,
    CAST(IFNULL(s.Six_Plus_Months, 0) AS UNSIGNED) AS TOTAL,
    CAST(IFNULL(s.Total_Patients, 0) AS UNSIGNED) AS Total_Patients,
    CAST(CASE 
        WHEN s.Total_Patients > 0 
        THEN ROUND((s.Six_Plus_Months * 100.0 / s.Total_Patients), 2)
        ELSE 0.00 
    END AS DECIMAL(5,2)) AS Percentage,
    -- Numerators: patients with MMD ≥6 months
    CAST(IFNULL(s.Male_0_14_6M_Plus, 0) AS UNSIGNED) AS Male_0_14,
    CAST(IFNULL(s.Male_0_14_6M_Plus, 0) AS UNSIGNED) AS Male_0_14_6M_Plus,
    CAST(IFNULL(s.Female_0_14_6M_Plus, 0) AS UNSIGNED) AS Female_0_14,
    CAST(IFNULL(s.Female_0_14_6M_Plus, 0) AS UNSIGNED) AS Female_0_14_6M_Plus,
    CAST(IFNULL(s.Male_over_14_6M_Plus, 0) AS UNSIGNED) AS Male_over_14,
    CAST(IFNULL(s.Male_over_14_6M_Plus, 0) AS UNSIGNED) AS Male_over_14_6M_Plus,
    CAST(IFNULL(s.Female_over_14_6M_Plus, 0) AS UNSIGNED) AS Female_over_14,
    CAST(IFNULL(s.Female_over_14_6M_Plus, 0) AS UNSIGNED) AS Female_over_14_6M_Plus,
