...
mmd_stats AS (
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN ndays BETWEEN 101 AND 130 THEN ClinicID END) AS Four_Months,
        ...
        COUNT(DISTINCT CASE WHEN type = 'Child' AND Sex = 'Male' AND ndays BETWEEN 101 AND 130 THEN ClinicID END) AS Male_0_14_4M,
        COUNT(DISTINCT CASE WHEN type = 'Child' AND Sex = 'Female' AND ndays BETWEEN 101 AND 130 THEN ClinicID END) AS Female_0_14_4M,
        COUNT(DISTINCT CASE WHEN type = 'Adult' AND Sex = 'Male' AND ndays BETWEEN 101 AND 130 THEN ClinicID END) AS Male_over_14_4M,
        COUNT(DISTINCT CASE WHEN type = 'Adult' AND Sex = 'Female' AND ndays BETWEEN 101 AND 130 THEN ClinicID END) AS Female_over_14_4M
...
SELECT
    ...
    -- Numerators: patients with MMD 4 months
    CAST(IFNULL(s.Male_0_14_4M, 0) AS UNSIGNED) AS Male_0_14,
    CAST(IFNULL(s.Male_0_14_4M, 0) AS UNSIGNED) AS Male_0_14_4M,
    CAST(IFNULL(s.Female_0_14_4M, 0) AS UNSIGNED) AS Female_0_14,
    CAST(IFNULL(s.Female_0_14_4M, 0) AS UNSIGNED) AS Female_0_14_4M,
    CAST(IFNULL(s.Male_over_14_4M, 0) AS UNSIGNED) AS Male_over_14,
    CAST(IFNULL(s.Male_over_14_4M, 0) AS UNSIGNED) AS Male_over_14_4M,
    CAST(IFNULL(s.Female_over_14_4M, 0) AS UNSIGNED) AS Female_over_14,
