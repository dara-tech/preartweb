-- ===================================================================
-- Indicator 9d: Percentage of ART patients with MMD 5 months
-- ===================================================================
...
mmd_stats AS (
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN ndays BETWEEN 131 AND 160 THEN ClinicID END) AS Five_Months,
        ...
        COUNT(DISTINCT CASE WHEN type = 'Child' AND Sex = 'Male' AND ndays BETWEEN 131 AND 160 THEN ClinicID END) AS Male_0_14_5M,
        COUNT(DISTINCT CASE WHEN type = 'Child' AND Sex = 'Female' AND ndays BETWEEN 131 AND 160 THEN ClinicID END) AS Female_0_14_5M,
        COUNT(DISTINCT CASE WHEN type = 'Adult' AND Sex = 'Male' AND ndays BETWEEN 131 AND 160 THEN ClinicID END) AS Male_over_14_5M,
        COUNT(DISTINCT CASE WHEN type = 'Adult' AND Sex = 'Female' AND ndays BETWEEN 131 AND 160 THEN ClinicID END) AS Female_over_14_5M
    FROM active_patients
)

SELECT
    '9d. Percentage of ART patients with MMD 5 months' AS Indicator,
    CAST(IFNULL(s.Five_Months, 0) AS UNSIGNED) AS TOTAL,
    CAST(IFNULL(s.Total_Patients, 0) AS UNSIGNED) AS Total_Patients,
    CAST(CASE 
        WHEN s.Total_Patients > 0 
        THEN ROUND((s.Five_Months * 100.0 / s.Total_Patients), 2)
        ELSE 0.00 
    END AS DECIMAL(5,2)) AS Percentage,
    -- Numerators: patients with MMD 5 months
    CAST(IFNULL(s.Male_0_14_5M, 0) AS UNSIGNED) AS Male_0_14,
    CAST(IFNULL(s.Male_0_14_5M, 0) AS UNSIGNED) AS Male_0_14_5M,
    CAST(IFNULL(s.Female_0_14_5M, 0) AS UNSIGNED) AS Female_0_14,
    CAST(IFNULL(s.Female_0_14_5M, 0) AS UNSIGNED) AS Female_0_14_5M,
    CAST(IFNULL(s.Male_over_14_5M, 0) AS UNSIGNED) AS Male_over_14,
    CAST(IFNULL(s.Male_over_14_5M, 0) AS UNSIGNED) AS Male_over_14_5M,
    CAST(IFNULL(s.Female_over_14_5M, 0) AS UNSIGNED) AS Female_over_14,
    CAST(IFNULL(s.Female_over_14_5M, 0) AS UNSIGNED) AS Female_over_14_5M,
