-- ===================================================================
-- Indicator 9a: Percentage of ART patients with MMD <3 months
-- ===================================================================
...
mmd_stats AS (
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN ndays <= 80 THEN ClinicID END) AS Less_Than_3M,
        ...
        COUNT(DISTINCT CASE WHEN type = 'Child' AND Sex = 'Male' AND ndays <= 80 THEN ClinicID END) AS Male_0_14_Less_3M,
        COUNT(DISTINCT CASE WHEN type = 'Child' AND Sex = 'Female' AND ndays <= 80 THEN ClinicID END) AS Female_0_14_Less_3M,
        COUNT(DISTINCT CASE WHEN type = 'Adult' AND Sex = 'Male' AND ndays <= 80 THEN ClinicID END) AS Male_over_14_Less_3M,
        COUNT(DISTINCT CASE WHEN type = 'Adult' AND Sex = 'Female' AND ndays <= 80 THEN ClinicID END) AS Female_over_14_Less_3M
    FROM active_patients
)

SELECT
    '9a. Percentage of ART patients with MMD <3 months' AS Indicator,
    CAST(IFNULL(s.Less_Than_3M, 0) AS UNSIGNED) AS TOTAL,
    CAST(IFNULL(s.Total_Patients, 0) AS UNSIGNED) AS Total_Patients,
    CAST(CASE 
        WHEN s.Total_Patients > 0 
        THEN ROUND((s.Less_Than_3M * 100.0 / s.Total_Patients), 2)
        ELSE 0.00 
    END AS DECIMAL(5,2)) AS Percentage,
    -- Numerators: patients with MMD <3 months
    CAST(IFNULL(s.Male_0_14_Less_3M, 0) AS UNSIGNED) AS Male_0_14,
    CAST(IFNULL(s.Male_0_14_Less_3M, 0) AS UNSIGNED) AS Male_0_14_Less_3M,
    CAST(IFNULL(s.Female_0_14_Less_3M, 0) AS UNSIGNED) AS Female_0_14,
    CAST(IFNULL(s.Female_0_14_Less_3M, 0) AS UNSIGNED) AS Female_0_14_Less_3M,
    CAST(IFNULL(s.Male_over_14_Less_3M, 0) AS UNSIGNED) AS Male_over_14,
    CAST(IFNULL(s.Male_over_14_Less_3M, 0) AS UNSIGNED) AS Male_over_14_Less_3M,
    CAST(IFNULL(s.Female_over_14_Less_3M, 0) AS UNSIGNED) AS Female_over_14,
    CAST(IFNULL(s.Female_over_14_Less_3M, 0) AS UNSIGNED) AS Female_over_14_Less_3M,
