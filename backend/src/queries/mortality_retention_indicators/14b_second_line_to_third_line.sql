...
SELECT
    ...
    CAST(COUNT(DISTINCT CASE WHEN SwitchStatus = 'Switched' THEN ClinicID END) AS UNSIGNED) AS Switched_To_Third_Line,
    CAST(COUNT(DISTINCT CASE WHEN SwitchStatus IN ('Switched', 'Not_Switched') THEN ClinicID END) AS UNSIGNED) AS Eligible_Patients,
    CAST(COUNT(DISTINCT ClinicID) AS UNSIGNED) AS Total_Second_Line_With_Consecutive_High_VL,
    CAST(CASE 
