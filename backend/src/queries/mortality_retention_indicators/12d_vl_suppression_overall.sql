...
SELECT
    ...
    CAST(SUM(IF(vlresultstatus = 'VL-Suppression' AND DateResult IS NOT NULL AND VLdostatus = 'Do_VL_in_12M' AND CAST(HIVLoad AS UNSIGNED) < 1000, 1, 0)) AS UNSIGNED) AS VL_Suppressed_Overall,
    ...
    CAST(CASE 
        WHEN COUNT(*) > 0 
        THEN ROUND((SUM(IF(vlresultstatus = 'VL-Suppression' AND DateResult IS NOT NULL AND VLdostatus = 'Do_VL_in_12M' AND CAST(HIVLoad AS UNSIGNED) < 1000, 1, 0)) * 100.0 / COUNT(*)), 2)
        ELSE 0.00 
