...
SELECT
    ...
    CAST(COUNT(DISTINCT CASE WHEN tptstatus = 'TPT Complete' THEN clinicid END) AS UNSIGNED) AS TPT_Completed,
    CAST(COUNT(DISTINCT CASE WHEN tptstatus = 'TPT Complete' THEN clinicid END) AS UNSIGNED) AS TOTAL,
    CAST(COUNT(DISTINCT clinicid) AS UNSIGNED) AS Total_ART_Patients,
    CAST(CASE 
        WHEN COUNT(DISTINCT clinicid) > 0 
        THEN ROUND((COUNT(DISTINCT CASE WHEN tptstatus = 'TPT Complete' THEN clinicid END) * 100.0 / COUNT(DISTINCT clinicid)), 2)
        ELSE 0.00 
    END AS DECIMAL(5,2)) AS Percentage,
    -- Numerators: patients who completed TPT
    CAST(COUNT(DISTINCT CASE WHEN Sex = 1 AND typepatients = '≤14' AND tptstatus = 'TPT Complete' THEN clinicid END) AS UNSIGNED) AS Male_0_14,
    CAST(COUNT(DISTINCT CASE WHEN Sex = 1 AND typepatients = '≤14' AND tptstatus = 'TPT Complete' THEN clinicid END) AS UNSIGNED) AS Male_0_14_TPT_Completed,
    CAST(COUNT(DISTINCT CASE WHEN Sex = 0 AND typepatients = '≤14' AND tptstatus = 'TPT Complete' THEN clinicid END) AS UNSIGNED) AS Female_0_14,
    CAST(COUNT(DISTINCT CASE WHEN Sex = 0 AND typepatients = '≤14' AND tptstatus = 'TPT Complete' THEN clinicid END) AS UNSIGNED) AS Female_0_14_TPT_Completed,
    CAST(COUNT(DISTINCT CASE WHEN Sex = 1 AND typepatients = '15+' AND tptstatus = 'TPT Complete' THEN clinicid END) AS UNSIGNED) AS Male_over_14,
    CAST(COUNT(DISTINCT CASE WHEN Sex = 1 AND typepatients = '15+' AND tptstatus = 'TPT Complete' THEN clinicid END) AS UNSIGNED) AS Male_over_14_TPT_Completed,
    CAST(COUNT(DISTINCT CASE WHEN Sex = 0 AND typepatients = '15+' AND tptstatus = 'TPT Complete' THEN clinicid END) AS UNSIGNED) AS Female_over_14,
    CAST(COUNT(DISTINCT CASE WHEN Sex = 0 AND typepatients = '15+' AND tptstatus = 'TPT Complete' THEN clinicid END) AS UNSIGNED) AS Female_over_14_TPT_Completed,
