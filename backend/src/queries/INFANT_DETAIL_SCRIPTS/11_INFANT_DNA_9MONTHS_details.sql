-- =====================================================
-- INFANT DNA PCR AT 9 MONTHS (Simplified)
-- =====================================================
...
FROM (
    ...
    AND n.DNAPcr = 5  -- Filter to 9 months tests only
    
    UNION
    
    -- Part 2: Visits from tblevmain where DNA=5 and (no matching test OR matching test has null result) (waiting)
    ...
    FROM (
        ...
        AND ev.DNA = 5  -- Filter to 9 months visits only
    ) req
    LEFT JOIN (
        SELECT t.*, 
