-- =====================================================
-- INFANT DNA PCR CONFIRMATORY DETAILS
-- =====================================================
...
-- =====================================================
-- MAIN QUERY
-- DNA PCR Confirmatory Test (DNAPcr = 4)
-- Matches logic from Rinfants.vb line 412-484: CmdPCRAllStage query
-- Shows ALL confirmatory tests - filter by Con_At, sex, and result in application
-- Part 1: Tests from tbletest where DNAPcr = 4
-- Part 2: Visits from tblevmain where DNA = 4 and waiting for results
-- =====================================================
SELECT DISTINCT
  ...
  4 AS dna_test_type,
  CASE
    WHEN c.Con_At = 0 THEN 'Confirmatory at Birth'
    WHEN c.Con_At = 1 THEN 'Confirmatory at 4-6 Weeks'
    WHEN c.Con_At = 5 THEN 'Confirmatory at 9 Months'
    WHEN c.Con_At = 3 AND c.Con_OI = 'True' THEN 'Confirmatory at OI'
    WHEN c.Con_At = 3 THEN 'Confirmatory Other'
    ELSE 'Confirmatory'
  ...
  CASE WHEN TIMESTAMPDIFF(DAY, c.DaBirth, COALESCE(c.DatTestArr, c.DatVisit)) < 31 THEN CONCAT(CAST(TIMESTAMPDIFF(DAY, c.DaBirth, COALESCE(c.DatTestArr, c.DatVisit)) AS CHAR), ' days') WHEN TIMESTAMPDIFF(DAY, c.DaBirth, COALESCE(c.DatTestArr, c.DatVisit)) < 365 THEN CONCAT(CAST(FLOOR(TIMESTAMPDIFF(DAY, c.DaBirth, COALESCE(c.DatTestArr, c.DatVisit))/30) AS CHAR), ' mo') ELSE CONCAT(CAST(FLOOR(TIMESTAMPDIFF(DAY, c.DaBirth, COALESCE(c.DatTestArr, c.DatVisit))/365) AS CHAR), ' yr') END AS age_at_test,
  'Infant' AS patient_type
FROM (
    -- Part 1: Tests from tbletest where DNAPcr = 4 and DatTestArr is in date range
    -- Matches VB code: Select n.ClinicID,ei.Sex,n.DNAPcr,...,If(n.Rn=r.Rn + 1 And n.DNAPcr=4, r.DNAPcr, null) As Con_At
    SELECT DISTINCT
        n.ClinicID,
        ei.Sex,
        ei.DaBirth,
    ...
    -- Part 2: Visits from tblevmain where DNA = 4 and (no matching test OR matching test has null result) (waiting)
    -- Matches VB code: select req.ClinicID, ei.Sex, req.DNA, ..., if(req.Rn=st.Rn+1 And req.DNA=4, st.DNA,null) Con_At
    SELECT DISTINCT
        req.ClinicID,
        ei.Sex,
        ei.DaBirth,
        ...
        CAST(IF(req.Rn = st.Rn + 1 AND req.DNA = 4 AND req.OI = 'True', st.OI, NULL) AS CHAR CHARACTER SET utf8mb4) COLLATE utf8mb4_general_ci AS Con_OI,
        et.Result
    FROM (
    ...
    WHERE et.Result IS NULL  -- Match VB code: where et.Result Is null
) c
