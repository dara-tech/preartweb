-- =====================================================
-- INFANT DNA PCR CONFIRMATORY OTHER DETAILS
-- =====================================================
...
-- =====================================================
-- MAIN QUERY
-- DNA PCR Confirmatory Test for Other (DNAPcr = 4, Con_At = 3, Con_OtherDNA != 'OI' or NULL)
-- Matches logic from Rinfants.vb line 650-668: c.DNA = 4 and c.Con_At = 3 and (c.Con_OtherDNA IS NULL OR c.Con_OtherDNA != 'OI')
-- =====================================================
SELECT DISTINCT
  ...
  4 AS dna_test_type,
  'Confirmatory Other' AS dna_test_display,
  c.Con_OtherDNA AS other_dna,
  c.Con_At AS Con_At,
  c.Result AS result,
  CASE
...
FROM (
    -- Part 1: Tests from tbletest where DNAPcr = 4 and Con_At = 3 (but not OI)
    SELECT DISTINCT
        n.ClinicID,
        ei.Sex,
        ei.DaBirth,
        ...
        n.DatTestArr,
        NULL AS DatVisit,
        CAST(IF(n.Rn = r.Rn + 1 AND n.DNAPcr = 4, r.OI, NULL) AS CHAR CHARACTER SET utf8mb4) COLLATE utf8mb4_general_ci AS Con_OtherDNA,
    ...
    -- Part 2: Visits from tblevmain where DNA = 4 and Con_At = 3 and Con_OtherDNA != 'OI' (waiting)
    SELECT DISTINCT
        req.ClinicID,
        ei.Sex,
        ei.DaBirth,
    ...
    LEFT JOIN (
        SELECT ev.ClinicID, ev.DatVisit, ev.DNA,
               CAST(ev.OtherDNA AS CHAR CHARACTER SET utf8mb4) COLLATE utf8mb4_general_ci AS OtherDNA,
               (SELECT COUNT(*) + 1 FROM tblevmain es 
                WHERE es.DatVisit BETWEEN @StartDate AND @EndDate
                AND es.DNA <> -1 AND es.ClinicID = ev.ClinicID AND es.DatVisit < ev.DatVisit 
                ORDER BY es.ClinicID, es.DatVisit) AS Rn
...
WHERE c.Con_At IS NOT NULL AND c.Con_At = 3 AND (c.Con_OtherDNA IS NULL OR c.Con_OtherDNA != 'OI')
